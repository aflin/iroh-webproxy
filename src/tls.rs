...
pub enum TlsMode {
    SelfSigned { host_suffixes: Vec<String> },
    LetsEncrypt(String),
...
pub fn build_tls_config(mode: &TlsMode) -> Result<Arc<rustls::ServerConfig>> {
    let (certs, key) = match mode {
        TlsMode::SelfSigned { host_suffixes } => build_self_signed(host_suffixes)?,
...
fn build_self_signed(host_suffixes: &[String]) -> Result<(Vec<CertificateDer<'static>>, PrivateKeyDer<'static>)> {
    ...
    for suffix in host_suffixes {
        if suffix != "localhost" {
            subject_alt_names.push(format!("*.{}", suffix));
            subject_alt_names.push(suffix.to_string());
        }
    }
